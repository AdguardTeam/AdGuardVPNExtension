---
description: Implement a feature according to its specification and plan
---
...
Implement a feature by executing the tasks defined in the implementation plan
(`{specs_dir}/plan.md`) according to the feature specification
(`{specs_dir}/spec.md`).

## Input

`$ARGUMENTS` is the text the user provides after the slash command. It may
contain instructions to control implementation scope. Examples:

- Task selection: "Task 1.1 only" or "Phase 1"
- Resume point: "Continue from Task 2.3"
- Skip tasks: "Skip tests for now"

If no arguments are provided, implement all tasks in order.
