...
Generate a structured implementation plan from a feature specification created
by `/sdd-spec`. The plan includes technical context, research findings, entity
definitions, API contracts, and actionable tasks.

## Input

`$ARGUMENTS` is the text the user provides after the slash command. It may
contain additional context or constraints for the implementation plan. Examples:
...
If no arguments are provided, proceed using only the feature spec and codebase
analysis.

## Prerequisites

Check for the existence of `{specs_dir}/spec.md`. If it does not exist, **STOP
immediately** and show this error:

**ERROR: Feature specification not found at `{specs_dir}/spec.md`.**

**A full feature specification created by `/sdd-spec` is required.**

**If you want a faster workflow for bug fixes or small changes, use
`/sdd-quickspec` instead, which creates a lightweight specification and skips
the separate planning phase.**

**To proceed:**

1. Run `/sdd-spec [your feature description]` to create a full specification
2. Then run `/sdd-plan` to create this implementation plan
...
2. **Output contract files**
   - Create `{specs_dir}/contracts/` directory if needed
...
1. **Create the implementation plan**
   - Extract the SDD version from the `version` value in the metadata comment
     at the top of this file
   - Write to `{specs_dir}/plan.md`
   - Use the template structure below
   - Replace all placeholders with concrete details

2. **Create contract files** (if applicable)
   - Write to `{specs_dir}/contracts/`
...
**Created**: [DATE]
**Status**: Draft
**Input**: Feature specification from `{specs_dir}/spec.md`
...
[Reference to contract files in `{specs_dir}/contracts/` or "N/A - no API
endpoints required"]
