---
description: Validate that a feature specification is fully implemented according to its plan
---
...
Verify that a feature specification has been fully implemented according to its
implementation plan. Both full specifications (spec.md + plan.md) and quick
specifications (quick.md) are supported.

## Prerequisites

Check for specification files in `{specs_dir}/`:

1. **Full spec mode**: Both `{specs_dir}/spec.md` AND `{specs_dir}/plan.md` exist
...
1. **Update spec and plan status** (if overall status is COMPLETE)
    - Change status from "Implemented" to "Validated" in `spec.md`
    - Change status from "Implemented" to "Validated" in `plan.md`
