...
if [ -z "$ARTIFACTS_ARG" ]; then
    ...
    # Convert comma-separated string to array to safely handle paths with spaces
...
# WARNING: This removes the entire workspace, including .git.
# It MUST only be run inside a disposable CI workspace — never in a local checkout.
