...
export class NotificationsStore {
    ...
    @action private addNotification = (notification: Notification): void => {
    ...
    @action public removeNotification = (notificationId: string): void => {
        this.notifications = this.notifications
            .filter((notification) => notification.id !== notificationId);
    };

    @action public notifySuccess = (message: ReactNode, action?: Action): void => {
        this.addNotification(new SuccessNotification(message, action));
    };

    @action public notifyError = (message: ReactNode, action?: Action): void => {
